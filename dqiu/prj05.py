...
def initialize_theta(bound):
    f1_W = np.random.uniform(-bound, bound, (N1, D))
    f1_b = np.random.uniform(-bound, bound, N1)
    f2_W = np.random.uniform(-bound, bound, (C, N1))
    f2_b = np.random.uniform(-bound, bound, C)
...
def forward(images, theta):
    ...
    # x = Flatten(images)
    x = images.astype(float).transpose(0,3,1,2).reshape((N, -1)) / 255
    # g = Linear_f1(x)
...
def backprop(labels, theta, z, h, g, x):
    ...
    for i in range(N):
        ...
        J = -np.exp(z[i][labels[i]] - max(z[i])) / sum(expz)
        p_z = expz/sum(expz)/N
        p_z[labels[i]] -= 1/N
        # z = Linear_f2(h)
        ...
        p_h = np.matmul(p_z, f2_W)
        p_f2_W += np.matmul(np.matrix(p_z).transpose(), np.matrix(h[i]))
        p_f2_b += p_z
        # h = ReLU(g)
        ...
        # ToDo: uncomment code below to add your own code
        p_f1_W += np.matmul(np.matrix(p_g).transpose(), np.matrix(x[i]))
        p_f1_b += p_g
...
def update_theta(theta, nabla_J, epsilon):
    ...
    (f1_W, f1_b, f2_W, f2_b) = theta
    updated_theta = (f1_W - p_f1_W * epsilon, f1_b - p_f1_b * epsilon, f2_W - p_f2_W * epsilon, f2_b - p_f2_b * epsilon)
...
accuracies_of_hyperparams = []
for bound in Bounds: # initial weight range
    for epsilon in Epsilons: # learning rate
        for batch_size in BatchSizes:
            # start training
            print("(bound, epsilon, batch_size) = (%f, %f, %d)" % (bound, epsilon, batch_size))
            start = time.time()
            ...
            accuracies_of_hyperparams.append((bound, epsilon, batch_size, count/validation_images.shape[0]))
